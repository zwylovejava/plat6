...
public class JobServiceImpl extends BaseServiceImpl<CltJob,CltJobExample,String> implements JobService {
    ...
    public Integer addTestManager(JobQuery jobQuery)throws Exception {
        ...
        try {
            ...
            if(addNum==1) {
                //指定成功,改变批次的状态
                cltBatch.setBatchId(cltJob.getBatchId());

                cltBatch.setStatus(CltJobStatus.COMPLETE.code());

                cltBatchMapper.updateByPrimaryKeySelective(cltBatch);
            }
            return addNum;
     ...
     */
    @Override
    public CltJob queryJobByJobId(JobQuery jobQuery)throws Exception {
    ...
    @Override
    public Integer countJobByManagerId(JobQuery jobQuery) throws Exception{
