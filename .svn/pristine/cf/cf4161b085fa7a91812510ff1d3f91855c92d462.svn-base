...
public class UserServiceImpl  extends BaseServiceImpl<CltUserLogin, CltUserLoginExample, String> implements UserService {
    ...
    public Page<CltUserAndLogin> queryUserList(UserAndLoginQuery userAndLoginQuery) throws Exception {
            ...
            List<CltUserAndLogin> users =null;

        try {

            users=cltUserLoginMapper.findAll(userAndLoginQuery);
    ...
    public Page<CltUserAndLogin> queryUserListEnable(UserAndLoginQuery userAndLoginQuery) throws Exception {
        ...
        try {
            ...
            users = cltUserLoginMapper.queryUserListEnable(userAndLoginQuery);
    ...
    public CltUserAndLogin findUserById(String userId) throws Exception {
        ...
        CltUserAndLogin cltUserAndLogin =null;

        try {

            cltUserAndLogin= cltUserLoginMapper.findUserById(userId);
    ...
    public CltUserAndLogin saveUser(UserAndLoginQuery userAndLoginQuery) throws Exception {
        ...
        //获取组装的用户的信息
        CltUserLogin cltUserLogin=getCltUserLoginInfo(userAndLoginQuery);

        CltUser cltUser=getCltUserInfo(userAndLoginQuery,cltUserLogin);
        ...
        try {
            ...
            //2.添加用户详细信息
            cltUserMapper.insertSelective(cltUser);

            //组装返回参数
            CltUserAndLogin  cltUserAndLogin = new CltUserAndLogin();

            BeanUtil.copyProperties(cltUser, cltUserAndLogin);

            BeanUtil.copyProperties(cltUserLogin, cltUserAndLogin);
    ...
    public Page<CltUserAndLogin> queryUserListByOrgId(UserAndLoginQuery userAndLoginQuery) throws Exception {
        ...
        try {
            ...
            users = cltUserLoginMapper.findUserByOrgId(userAndLoginQuery);
    ...
    //用户登录表信息
    public CltUserLogin getCltUserLoginInfo(UserAndLoginQuery userAndLoginQuery)throws Exception{
   ...
   //用户信息表信息
    public CltUser getCltUserInfo(UserAndLoginQuery userAndLoginQuery,CltUserLogin cltUserLogin)throws Exception{

        CltUser cltUser = new CltUser();

        BeanUtil.copyProperties(userAndLoginQuery, cltUser);

        cltUser.setUserId(cltUserLogin.getUserId());


        cltUser.setWorkStatus("1");

        cltUser.setCreateDate(new Date());
