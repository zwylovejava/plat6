...
public class CltFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    private void loadResourceDefine() {
        ...
        Page<CltRole> cltRolePage = roleFeignClient.queryAllRoleList(roleQuery).getResultData();

        List<CltRole> cltRoleList = cltRolePage.getList();

        if(cltRoleList!=null && cltRoleList.size()>0) {
            resourceMap = new HashMap<String, Collection<ConfigAttribute>>();
            for (CltRole role : cltRoleList) {
